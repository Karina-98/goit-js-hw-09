...
const options = {
    ...
    onClose (selectedDates) {
        if (selectedDates[0].getDate() < options.defaultDate) btn.removeAttribute("disabled");
        else {
            btn.disabled = true;
...
const fl = (0, _flatpickrDefault.default)(inputText, options);
function convertMs(ms) {
    ...
    const hours1 = Math.floor(ms % day1 / hour);
    const minutes1 = Math.floor(ms % day1 % hour / minute);
    const seconds1 = Math.floor(ms % day1 % hour % minute / second);
    return {
        days,
        hours: hours1,
        minutes: minutes1,
